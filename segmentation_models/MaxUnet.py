...
class MaxUnet(nn.Module):
    def __init__(self, in_channels=1, out_channels=4, blocks=64):
        ...
        self.encoder5 = EncoderBlock(8*blocks, 16*blocks)

        self.bottleneck = nn.Sequential(
            nn.Conv2d(16*blocks, 32*blocks, kernel_size=3, padding=1),
            nn.BatchNorm2d(32*blocks),
            nn.ReLU(inplace=True),
            nn.Conv2d(32*blocks, 32*blocks, kernel_size=3, padding=1),
            nn.BatchNorm2d(32*blocks),
            nn.ReLU(inplace=True),
            nn.Conv2d(32*blocks, 32*blocks, kernel_size=3, padding=1),
            nn.BatchNorm2d(32*blocks),
        ...
        self.decoder1 = DecoderBlock(32*blocks, 16*blocks, 16*blocks)
        self.decoder2 = DecoderBlock(16*blocks, 8*blocks, 8*blocks)
        self.decoder3 = DecoderBlock(8*blocks, 4*blocks, 4*blocks)
        self.decoder4 = DecoderBlock(4*blocks, 2*blocks, 2*blocks)
        self.decoder5 = DecoderBlock(2*blocks, blocks, blocks)
